defmodule Ecto.Druid.QueryTest do
  ...
  describe "array functions" do
    test "array/1" do
      sql = from(t in "test", select: array([1, 2, 3])) |> to_sql()
      assert sql == {"SELECT ARRAY[1, 2, 3] FROM \"test\" AS t0", []}
    end

    test "array_length/1" do
      sql = from(t in "test", select: array_length(t.array)) |> to_sql()
      assert sql == {"SELECT ARRAY_LENGTH(t0.\"array\") FROM \"test\" AS t0", []}
    end

    test "array_offset/2" do
      sql = from(t in "test", select: array_offset(t.array, 1)) |> to_sql()
      assert sql == {"SELECT ARRAY_OFFSET(t0.\"array\", 1) FROM \"test\" AS t0", []}
    end

    test "array_ordinal/2" do
      sql = from(t in "test", select: array_ordinal(t.array, 1)) |> to_sql()
      assert sql == {"SELECT ARRAY_ORDINAL(t0.\"array\", 1) FROM \"test\" AS t0", []}
    end

    test "array_contains/2" do
      sql = from(t in "test", select: array_contains(t.array, 1)) |> to_sql()
      assert sql == {"SELECT ARRAY_CONTAINS(t0.\"array\", 1) FROM \"test\" AS t0", []}
    end

    test "array_overlap/2" do
      sql = from(t in "test", select: array_overlap(t.array, array([1, 2, 3]))) |> to_sql()
      assert sql == {"SELECT ARRAY_OVERLAP(t0.\"array\", ARRAY[1, 2, 3]) FROM \"test\" AS t0", []}
    end

    test "array_offset_of/2" do
      sql = from(t in "test", select: array_offset_of(t.array, 1)) |> to_sql()
      assert sql == {"SELECT ARRAY_OFFSET_OF(t0.\"array\", 1) FROM \"test\" AS t0", []}
    end

    test "array_ordinal_of/2" do
      sql = from(t in "test", select: array_ordinal_of(t.array, 1)) |> to_sql()
      assert sql == {"SELECT ARRAY_ORDINAL_OF(t0.\"array\", 1) FROM \"test\" AS t0", []}
    end

    test "array_prepend/2" do
      sql = from(t in "test", select: array_prepend(1, t.array)) |> to_sql()
      assert sql == {"SELECT ARRAY_PREPEND(1, t0.\"array\") FROM \"test\" AS t0", []}
    end

    test "array_append/2" do
      sql = from(t in "test", select: array_append(t.array, 1)) |> to_sql()
      assert sql == {"SELECT ARRAY_APPEND(t0.\"array\", 1) FROM \"test\" AS t0", []}
    end

    test "array_concat/2" do
      sql = from(t in "test", select: array_concat(t.array, array([1, 2, 3]))) |> to_sql()
      assert sql == {"SELECT ARRAY_CONCAT(t0.\"array\", ARRAY[1, 2, 3]) FROM \"test\" AS t0", []}
    end

    test "array_slice/3" do
      sql = from(t in "test", select: array_slice(t.array, 1, 2)) |> to_sql()
      assert sql == {"SELECT ARRAY_SLICE(t0.\"array\", 1, 2) FROM \"test\" AS t0", []}
    end

    test "array_to_string/2" do
      sql = from(t in "test", select: array_to_string(t.array, ",")) |> to_sql()
      assert sql == {"SELECT ARRAY_TO_STRING(t0.\"array\", ',') FROM \"test\" AS t0", []}
    end

    test "string_to_array/2" do
      sql = from(t in "test", select: string_to_array("1,2,3", ",")) |> to_sql()
      assert sql == {"SELECT STRING_TO_ARRAY('1,2,3', ',') FROM \"test\" AS t0", []}
    end

    test "array_to_mv/1" do
      sql = from(t in "test", select: array_to_mv(t.array)) |> to_sql()
      assert sql == {"SELECT ARRAY_TO_MV(t0.\"array\") FROM \"test\" AS t0", []}
    end
  end

  describe "multi-value string functions" do
    test "mv_filter_only/2" do
      sql = from(t in "test", select: mv_filter_only(t.array, array([1, 2, 3]))) |> to_sql()

      assert sql ==
               {"SELECT MV_FILTER_ONLY(t0.\"array\", ARRAY[1, 2, 3]) FROM \"test\" AS t0", []}
    end

    test "mv_filter_none/2" do
      sql = from(t in "test", select: mv_filter_none(t.array, array([1, 2, 3]))) |> to_sql()

      assert sql ==
               {"SELECT MV_FILTER_NONE(t0.\"array\", ARRAY[1, 2, 3]) FROM \"test\" AS t0", []}
    end

    test "mv_length/1" do
      sql = from(t in "test", select: mv_length(t.array)) |> to_sql()
      assert sql == {"SELECT MV_LENGTH(t0.\"array\") FROM \"test\" AS t0", []}
    end

    test "mv_offset/2" do
      sql = from(t in "test", select: mv_offset(t.array, 1)) |> to_sql()
      assert sql == {"SELECT MV_OFFSET(t0.\"array\", 1) FROM \"test\" AS t0", []}
    end

    test "mv_ordinal/2" do
      sql = from(t in "test", select: mv_ordinal(t.array, 1)) |> to_sql()
      assert sql == {"SELECT MV_ORDINAL(t0.\"array\", 1) FROM \"test\" AS t0", []}
    end

    test "mv_contains/2" do
      sql = from(t in "test", select: mv_contains(t.array, 1)) |> to_sql()
      assert sql == {"SELECT MV_CONTAINS(t0.\"array\", 1) FROM \"test\" AS t0", []}
    end

    test "mv_overlap/2" do
      sql = from(t in "test", select: mv_overlap(t.array, array([1, 2, 3]))) |> to_sql()
      assert sql == {"SELECT MV_OVERLAP(t0.\"array\", ARRAY[1, 2, 3]) FROM \"test\" AS t0", []}
    end

    test "mv_offset_of/2" do
      sql = from(t in "test", select: mv_offset_of(t.array, 1)) |> to_sql()
      assert sql == {"SELECT MV_OFFSET_OF(t0.\"array\", 1) FROM \"test\" AS t0", []}
    end

    test "mv_ordinal_of/2" do
      sql = from(t in "test", select: mv_ordinal_of(t.array, 1)) |> to_sql()
      assert sql == {"SELECT MV_ORDINAL_OF(t0.\"array\", 1) FROM \"test\" AS t0", []}
    end

    test "mv_prepend/2" do
      sql = from(t in "test", select: mv_prepend(1, t.array)) |> to_sql()
      assert sql == {"SELECT MV_PREPEND(1, t0.\"array\") FROM \"test\" AS t0", []}
    end

    test "mv_append/2" do
      sql = from(t in "test", select: mv_append(t.array, 1)) |> to_sql()
      assert sql == {"SELECT MV_APPEND(t0.\"array\", 1) FROM \"test\" AS t0", []}
    end

    test "mv_concat/2" do
      sql = from(t in "test", select: mv_concat(t.array, array([1, 2, 3]))) |> to_sql()
      assert sql == {"SELECT MV_CONCAT(t0.\"array\", ARRAY[1, 2, 3]) FROM \"test\" AS t0", []}
    end

    test "mv_slice/3" do
      sql = from(t in "test", select: mv_slice(t.array, 1, 2)) |> to_sql()
      assert sql == {"SELECT MV_SLICE(t0.\"array\", 1, 2) FROM \"test\" AS t0", []}
    end

    test "mv_to_string/2" do
      sql = from(t in "test", select: mv_to_string(t.array, ",")) |> to_sql()
      assert sql == {"SELECT MV_TO_STRING(t0.\"array\", ',') FROM \"test\" AS t0", []}
    end

    test "string_to_mv/2" do
      sql = from(t in "test", select: string_to_mv("1,2,3", ",")) |> to_sql()
      assert sql == {"SELECT STRING_TO_MV('1,2,3', ',') FROM \"test\" AS t0", []}
    end

    test "mv_to_array/1" do
      sql = from(t in "test", select: mv_to_array("1,2,3")) |> to_sql()
